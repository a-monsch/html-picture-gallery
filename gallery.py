...
parser.add_argument(
    "--verbose", "-v", action="store_true", help="print some info to the screen"
)
parser.add_argument(
    "--title-size",
    default="100",
    help="scale the text size of the plot titles, expressed as a percentage [0-100]",
)
...
def ProcessDir(indir, is_parent=True, subdirs=[]):
    ...
    other_files = dict(
        (k, v)
        for k, v in all_base_files.items()
    ...
    for f, all_exts in sorted(base_files.items()):
        ...
        if args.verbose:
            print(info)
        elements += PLOT_ELE.format(
            file="%s.png" % f, tags=tags, extra=extra, fontsize=args.title_size
        )
    ...
    if len(groups) > 0:
        for key, val in groups.items():
            ...
            button_groups += BUTTON_GROUP.format(
                GROUP=CleanStr(key), TITLE=key, BUTTONS=buttons
            )
        ...
        for f, all_exts in sorted(other_files.items()):
            for ext in all_exts:
                ...
                if ext == ".md":
                    ...
                    other_html.append(
                        '  <li><a href="{FILEHTML}">{FILE}</a> <a href="{FILE}">[raw]</a></li>'.format(
                            FILEHTML=f + ".html", FILE=f + ext
                        )
                    )
                else:
                    other_html.append(
                        '  <li><a href="{FILE}">{FILE}</a></li>'.format(FILE=f + ext)
                    )
        if added > 0:
    ...
    if not is_parent or len(subdirs) > 0:
        links += (
            "<p><strong>Current directory:</strong> %s | <strong>Navigation:</strong> "
            % indir
        )
        if not is_parent:
...
def clean_directory(init_dir):
    for root, dirs, files in os.walk(init_dir):
        has_pdf_or_png = any(
            file.endswith(".pdf") or file.endswith(".png") for file in files
        )
        ...
        if not has_pdf_or_png:
            ...
            if os.path.isfile(xy_php_path):
                ...
                print(f"Removed {xy_php_path} from {root}")
